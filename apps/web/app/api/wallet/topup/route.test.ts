...
describe("POST /api/wallet/topup", () => {
  ...
  it("card top-up: one-time card-only hosted checkout, grossed-up charge, short orderReference", async () => {
    vi.mocked(getSession).mockResolvedValue(createMockSession({ id: "u1" }));
  ...
  it("returns 502 when the checkout call fails", async () => {
    vi.mocked(getSession).mockResolvedValue(createMockSession({ id: "u1" }));
    vi.mocked(createCheckoutOrder).mockRejectedValue(new Error("nomba down"));
    expect((await POST(req({ amountMinor: 1_000_000 }))).status).toBe(502);
  });
});
