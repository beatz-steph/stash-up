...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    // Only retry memberships that failed
    for (const membership of activeMemberships) {
      if (membership.vaProvisionStatus !== "FAILED") continue;
      ...
      try {
        ...
        console.error(`Failed to provision VA on retry for membership ${membership.id}:`, err);
        // Remains FAILED
      }
    }
